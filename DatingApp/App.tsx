      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          {pages.map((page, index) => (
            <View key={index} style={[{ width: width, height, paddingHorizontal: 16 }]}>
              <ImageBackground
              ...
              >
                <PageIndicator count={pages.length} current={animatedCurrent} variant={'train'} color={'white'} style={{ top: 15 }} dashSize={10} />
                <View style={{ flexDirection: 'row', bottom: 20, position: 'absolute', marginLeft: 16 }}>
                  {flatListData.map((item, index) => (
                    <GlassView glassStyle={{
                      paddingHorizontal: 12, paddingVertical: 8,
                      borderRadius: 16, marginRight: index !== flatListData.length - 1 ? 9 : 0, flexDirection: 'row', alignItems: 'center'
                    }}>
